...
#GRAB DATE AND RETURN A STRING WITH 'DID YOU KNOW THAT THIS HAPPENED ON YOUR BDAY?' 
            ...
            puts "I hate to be cold as".light_yellow "ICE".light_cyan "but we are going to have to do that again!".light_yellow
            self.get_birth_year
            end
        else
            puts "\n" * 35
            print "It's COOL".light_cyan
            print " #{users_name}".light_yellow
...
#STILL NEED TO PERSIST NEW USER
    ...
    def self.get_username
        username = PROMPT.ask("What do you want your new User Name to be?", required: true)
        yes_or_no = PROMPT.yes?("#{username.light_green.bold} is what you entered. Are you sure?") do |q|
          q.suffix 'Y/N'
        end
        if yes_or_no 
            username
